...
class ComicScraper(GeneralScraper):
    ...
    def comic(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            yield from ctx.bot.pm_help(ctx)

    @comic.command(name="random", pass_context=True)
    @asyncio.coroutine
    def comic_random(self, ctx):
        """Return random comic from a random comic website."""
        yield from random.choice(self.random_commands).invoke(ctx)

    @comic.command(name="latest", pass_context=True)
    @asyncio.coroutine
    def comic_latest(self, ctx):
        """Return the latest comic from the next comic website."""
        yield from self.latest_commands[self.current_comic].invoke(ctx)
...
#   Begin CommitStrip Section
    ...
    @asyncio.coroutine
    def commitstrip_random(self):
        """Return a random CommitStrip comic."""
        # Get the last page number
        message = yield from self.bot.say("Loading...")
        index_html = (yield from self.fetch_page("http://www.commitstrip.com/")).decode("utf-8")
        ...
        random_page = (yield from self.fetch_page(random_page_url)).decode("utf-8")
        ...
        comic = (yield from self.commitstrip_comic_from_url(comic_url))
        yield from self.bot.edit_message(message, "**Title**: `{}`\n"
                                             "**Image**: {}".format(comic[0],
                                                                    comic[1]))

    @commitstrip.command(name="latest")
    @asyncio.coroutine
    def commitstrip_latest(self):
        """Return the latest CommitStrip comic."""
        message = yield from self.bot.say("Loading...")
        index_html = (yield from self.fetch_page("http://www.commitstrip.com/")).decode("utf-8")
        url_http = index_html.split('<div class="excerpt">')[1].split('<a href="')[1].split('"')[0]
        comic = (yield from self.commitstrip_comic_from_url(url_http))
        yield from self.bot.edit_message(message, "**Title**: `{}`\n"
                                             "**Image**: {}".format(comic[0],
                                                                    comic[1]))
    ...
    def commitstrip_comic_from_url(self, comic_url):
        ...
        """
        comic_html = (yield from self.fetch_page(comic_url)).decode("utf-8")
...
#   Begin CubeDrone Section
    ...
    @asyncio.coroutine
    def cubedrone_random(self):
        """Return a random CubeDrone comic."""
        # Get the last page number
        message = yield from self.bot.say("Loading...")
        index_html = (yield from self.fetch_page("https://cube-drone.com/")).decode("utf-8")
        ...
        comic = (yield from self.cubedrone_comic_from_url(comic_url))
        yield from self.bot.edit_message(message, "**Title**: `{}`\n"
                                             "**Image**: {}".format(comic[0],
                                                                    comic[1]))
    ...
    @asyncio.coroutine
    def cubedrone_latest(self):
        """Return the latest CubeDrone comic."""
        message = yield from self.bot.say("Loading...")
        comic = (yield from self.cubedrone_comic_from_url("https://cube-drone.com/"))
        yield from self.bot.edit_message(message, "**Title**: `{}`\n"
                                             "**Image**: {}".format(comic[0],
                                                                    comic[1]))
    ...
    @asyncio.coroutine
    def cubedrone_comic_from_url(self, comic_url):
        """Return comic title and comic image url from comic page url.
